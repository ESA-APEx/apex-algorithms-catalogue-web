...
export const CatalogueCwlDetailParametersTable = ({
  cwlUrl,
}: CatalogueCwlDetailParametersTableProps) => {
  ...
  const [status, setStatus] = useState<
    "loading" | "error" | "success" | "protected"
  >("loading");
  ...
  const fetchDetail = async () => {
    ...
    try {
      ...
      console.error(
        `Error when fetching the cwl process definition: ${JSON.stringify(error)}`,
      );
