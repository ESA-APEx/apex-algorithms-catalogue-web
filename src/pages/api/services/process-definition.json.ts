...
export const GET: APIRoute = async ({ request }) => {
  ...
  if (!targetUrl) {
    return new Response(
      JSON.stringify({ message: "Parameter URL is required." }),
      {
        status: 400,
        headers,
      },
  ...
  if (!supportedTypes.includes(type as string)) {
    return new Response(
      JSON.stringify({ message: `Type ${type} is not supported.` }),
      {
        status: 400,
        headers,
      },
  ...
  try {
    ...
    const cwlGraph = parsed["$graph"]?.find(
      (graph: any) => graph.id === "main",
    ) as CwlWorkflow | undefined;
