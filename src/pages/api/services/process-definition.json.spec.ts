...
describe('API Route: GET', () => {
	...
	it('should return 400 if no URL is provided', async () => {
		const request = new Request('http://localhost:4321/api/services/process-definition.json?type=cwl');
	...
	it('should return 400 if unsupported type is provided', async () => {
		const request = new Request('http://localhost:4321/api/services/process-definition.json?url=http://example.com/file.cwl&type=unknown');
		const response = await GET({ request } as any);
	...
	it('should return 500 if fetch throws error', async () => {
		...
		const json = await response.json();
		expect(json).toEqual({
			message: 'Fetching process definition for url http://example.com/file.cwl has failed.',
		});
	});
});
